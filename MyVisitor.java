...
public class MyVisitor extends guqinBaseVisitor<Boolean> {
	...
	public Boolean visitFuncall(guqinParser.FuncallContext ctx) {
		if (ctx.dimensions_choose(0) == null) {// it's a big function.
			...
			for (int i = 0; i < to_check.size(); i++) {
				...
				if (to_check.get(i).type.compareTo(demand.get(i).type) != 0) {
					return false;
				}
			}
			...
			for (int i = 0; i < to_check.size(); i++) {
				if (to_check.get(i).type.compareTo(desired_args.get(i).type) != 0) {
					return false;
				}
	...
	public Boolean visitAssignexpr(guqinParser.AssignexprContext ctx) {
		...
		if (!node_type.equals(l_type.type)) {
			return false;
		}
	...
	public Boolean visitFormat_string(guqinParser.Format_stringContext ctx) {
		for (int i = 0; i < ctx.getChildCount(); i++) {
			...
			if (ctx.getChild(i) instanceof guqinParser.ExprContext) {
				if ((!node_type.equals("int")) && (!node_type.equals("string")) && (!node_type.equals("bool"))) {
					return false;
				}
			}
		}
	...
	public Boolean visitArray_new(guqinParser.Array_newContext ctx) {
		...
		if (!node_type.equals(desired_type)) {
			return false;
		}
		return true;
	}
	...
	public Boolean visitOrder(guqinParser.OrderContext ctx) {
		...
		if ((!node_type.equals("int")) && (!node_type.equals("string"))) {
			return false;
		}
		dim = 0;
		node_type = "bool";
